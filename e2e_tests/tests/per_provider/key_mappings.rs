...
use parsec_client::core::interface::requests::{ResponseStatus, Opcode};
    ...
    for key in keys {
        if key.name == "rsa" {
            let ciphertext = client
                .asymmetric_encrypt_message_with_rsapkcs1v15(key.name.clone(), PLAINTEXT_MESSAGE.to_vec())
                .unwrap();
            ...
            panic!("The key {} should have been deleted as a fake mapping was created for it.", key.name);
        }

        if client.is_operation_supported(Opcode::PsaExportKey) {
            assert_eq!(client.export_key(key.name.clone()).unwrap_err(), ResponseStatus::PsaErrorNotPermitted);
        }
    }
